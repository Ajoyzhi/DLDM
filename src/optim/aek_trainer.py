...
AJoy
AE+K-means的实现代码
    train():通过均方误差对AE进行训练，仅使用正常数据
    test()：对训练集（正常数据）的重构误差利用K-Means聚类，得到正常数据的簇中心和半径
            对测试集，利用与簇中心的曼哈顿距离判断数据异常与否
"""
    ...
    def test(self, dataset: BaseADDataset, ae_net: BaseNet, flg=0):
        ...
        if flg == 1:  # 训练集
            ...
            for i in range(len(pred_km)):
                # AJoy 计算每一个点（重构误差）到簇中心的距离（曼哈顿距离）
